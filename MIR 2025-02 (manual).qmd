...
reporting_month <- "Feb" # <- replace this 

#Select from the following  
#Jan,Feb,Mar,Apr,May,Jun,Jul,Aug,Sep,Oct,Nov,Dec


# Define Past 3 months
p3m <- c("Dec24","Jan","Feb") # <- replace this 
...
### \<SPECIAL FOR JANUARY\> Step 4.4: Computing for the average monthly consumption (site-level) {data-link="Step 1.3: Extract facility names from General Input Sheet"}
...
disp <- disp %>%
  ...
  mutate(
    ...
    amc_3_basic = ifelse(is.nan(round(sum/count)),0,round(sum/count, 1)),
...
# STEP 8: Generate national and regional summary -----------------------------------------------

# Regional
disp_reg <- disp %>%
  group_by(region, numeric_unit, product) %>%
  summarise(across(all_of(month_order), sum, na.rm = TRUE), .groups = "drop") %>% # Ensure proper grouping
  mutate(across(all_of(month_order), ~ ifelse(. == 0, NA, .))) %>%
  mutate(
    sum = rowSums(select(., all_of(p3m)), na.rm = TRUE),
    count = rowSums(!is.na(select(., all_of(p3m)))),
...
print("Done generating regional and national dispensing dataframe.")
```
