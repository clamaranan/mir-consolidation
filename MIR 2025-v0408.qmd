...
fefo_inventory <- function(data) {
  ...
  data %>%
    ...
    mutate(
      ...
      # Wasted stock (if expired)
      wastage = ifelse(expiry_end < (analysis_start_date + months(6)) & remaining_stock > 0, remaining_stock, 0),  
...
fefo_projection <- fefo_projection %>%
  select(region, FacilityName, hub, filename, product, numeric_unit, unit_description, expiry,expiry_end, reporting_month, filename, amc_3_basic, fefo_consumed, remaining_stock, wastage, will_expire) %>%
...
fefo_projection_natl_product <- fefo_projection %>%
...
data_frames <- list(
  ...
  "Proj Consumption - Natl (prod)" = fefo_projection_natl_product,
